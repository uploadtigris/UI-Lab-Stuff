...
def main():
    avg_prices = pd.read_csv(r'/Users/tigris/Documents/UI Lab/Jun EV Task/Correlation_Analysis/CSVs\TotalVehiclesByMake_Chevy_ONLY.csv')
    print(avg_prices)
    ...
    # avg_prices_phev = avg_prices_phev[(avg_prices_phev['AVG_PRICE_PHEV'] < 110000)]
    
    if len(avg_prices_phev) >= 2:
        corr_phev, corr_phev_pvalue = pearsonr(avg_prices_phev['NUM_PHEV'], avg_prices_phev['AVG_PRICE_PHEV'])
        corr_phev = round(corr_phev, 4)
        print("\nThe Pearson Correlation Coefficient for the number of PHEV and Average Price by Zipcode:", corr_phev)
        ...
        plt.title('Average PHEV Price vs. Number of PHEV Vehicles Chevrolet ONLY')
        ...
        print("Insufficient data for PHEV correlation.")
    
    # For BEV
    avg_prices_bev = avg_prices[(avg_prices['NUM_BEV'] != 0) & (avg_prices['AVG_PRICE_BEV'] != 0)]

    # # REMOVING OUTLIERS (below)
    # avg_prices_bev = avg_prices_bev[(avg_prices_bev['AVG_PRICE_TOTAL_EV'] < 110000)]
    
    if len(avg_prices_bev) >= 2:
        corr_bev, corr_bev_pvalue = pearsonr(avg_prices_bev['NUM_BEV'], avg_prices_bev['AVG_PRICE_BEV'])
        corr_bev = round(corr_bev, 4) 
        print("\nThe Pearson Correlation Coefficient for the number of BEV and Average Price by Zipcode:", corr_bev)
        ...
        plt.title('Average BEV Price vs. Number of BEV Vehicles Chevrolet ONLY')
        ...
        print("Insufficient data for BEV correlation.")
    ...
    if len(avg_prices_total) >= 2:
        ...
        # Calculate mean and median for AVG_PRICE_PHEV
        mean_total_ev = avg_prices_total['AVG_PRICE_TOTAL_EV'].mean()
        median_total_ev = avg_prices_total['AVG_PRICE_TOTAL_EV'].median()
        ...
        plt.title('Overall Average Price vs. Number of TOTAL EV Vehicles Chevrolet ONLY')
        ...
        print("Insufficient data for Total EV correlation.")
