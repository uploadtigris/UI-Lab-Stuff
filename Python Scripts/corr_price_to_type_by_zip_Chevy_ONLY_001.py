...
def main():
    ...
    new_merged_df = merged_df[(merged_df['MAKE'] == 'Chevrolet')]
    ...
    file_path = '/Users/tigris/Documents/UI Lab/Jun EV Task/Correlation_Analysis/CSVs\TotalVehiclesByMake_Chevy_ONLY.csv'
    ...
    with open(file_path, 'w', newline='') as file:
        ...
        for index, row in merged_result_df.iterrows():
            ...
            phev_price_rows = phev_price_avg[phev_price_avg['Zip Code'] == zipcode]
            if not phev_price_rows.empty:
                avg_price_phev = phev_price_rows['AVG_PRICE'].values[0]
            else:
             avg_price_phev = 0

            bev_price_rows = bev_price_avg[bev_price_avg['Zip Code'] == zipcode]
            if not bev_price_rows.empty:
                avg_price_bev = bev_price_rows['AVG_PRICE'].values[0]
            else:
             avg_price_bev = 0
