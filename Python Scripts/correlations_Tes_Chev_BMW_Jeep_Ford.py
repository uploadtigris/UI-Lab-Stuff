...
def main():
    avg_prices = pd.read_csv(r'/Users/tigris/Documents/UI Lab/Jun EV Task/Correlation_Analysis/CSVs\TotalVehiclesByMake_TCBJF.csv')
    print(avg_prices)
    ...
    if len(avg_prices_phev) >= 2:
        ...
        plt.title('Average PHEV Price vs. Number of PHEV Vehicles TCBJF')
    ...
    if len(avg_prices_bev) >= 2:
        ...
        plt.title('Average BEV Price vs. Number of BEV Vehicles TCBJF')
    ...
    if len(avg_prices_total) >= 2:
        ...
        plt.title('Overall Average Price vs. Number of TOTAL EV Vehicles for TCBJF')
